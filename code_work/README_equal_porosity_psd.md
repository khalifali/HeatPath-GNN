...
### Important correction for v4 packing outputs

The v4 LAMMPS script used `%d` for atom IDs returned by a local compute. LAMMPS
stores these compute values as floating point, so the contact rows contain
correct distances and forces but zero particle IDs. Regenerate only the contact
file from each existing restart:
...
This runs no additional packing dynamics. It creates
`contacts_final_with_ids.dump`. Use that file with the solver:

```bash
python3 solve_packing_heat_transfer.py packing_seed_18427 \
  --contacts contacts_final_with_ids.dump \
  --k-low 1.0 --k-fluid 0.0
```

The corrected `in.generate_equal_porosity_psd_v5.lammps` writes valid contact
IDs directly for any future packing simulations.
