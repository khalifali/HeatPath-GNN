...
All three methods produce particle selections, not temperature predictions. We assign high conductivity to the selected fifty particles and low conductivity to the remaining four hundred and fifty, then solve the thermal problem. The degree and heat-throughput rules give one allocation each. Random sampling gives one hundred allocations, and its reported conductivity is their mean. Quotas are applied separately within each size class. The plots retain the original Heat path label, but this method is a ranking by baseline heat throughput, not a shortest-path algorithm.
...
## 10. Training and later use have different workflows
...
## 11. Baseline physics before material allocation
...
## 12. The GNN uses geometry and the baseline thermal solution
...
## 13. Training targets describe how often search selects each particle
...
## 14. Scoring assigns priorities, then ranking enforces the budget
...
## 15. Each test packing is withheld from model fitting
...
## 16. Gain and recovery answer different questions
...
## 17. GNN improves conductivity by 30\% over random
...
## 18. Absolute conductivity remains below the search reference
...
## 19. GNN beats MLP on 29 of 30 held-out packings
...
## 20. Contact-network diagnostics help interpret allocation
...
## 21. Numerical checks pass, but contact sizes need scrutiny
...
## 22. The numerical overlap floor has little influence
...
## 23. Fast scoring is one part of the complete prediction procedure
...
## 24. Conclusions and next research steps
