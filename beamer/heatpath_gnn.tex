...
\centering
\begin{tikzpicture}[flow/.style={draw=tumblue!65,fill=tumlight,rounded corners=2pt,
text width=2.10cm,minimum height=1.5cm,align=center,font=\scriptsize,inner sep=4pt}]
\node[flow] (a) at (0,0) {New DEM packing\\Fixed contact graph};
\node[flow] (b) at (2.75,0) {All-low thermal solve\\Build particle and\\contact features};
\node[flow] (c) at (5.50,0) {Trained GNN\\One score for\\every particle};
\node[flow] (d) at (8.25,0) {Rank separately\\within each size class\\Select 10 / 30 / 10};
\node[flow] (e) at (11.00,0) {Final thermal solve\\Verify $k_{\rm eff}$};
\draw[arr](a)--(b);\draw[arr](b)--(c);\draw[arr](c)--(d);\draw[arr](d)--(e);
\end{tikzpicture}
\vspace{.45cm}
\begin{columns}[T,onlytextwidth]
