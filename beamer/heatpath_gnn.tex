...
\end{frame}
\begin{frame}{Where the methods enter training, prediction and evaluation}
\small\centering
\begin{tikzpicture}[flow/.style={draw=tumblue!65,fill=tumlight,rounded corners=2pt,text width=2.8cm,minimum height=1.45cm,align=center,font=\scriptsize,inner sep=4pt}]
\node[anchor=west,tumblue,font=\small\bfseries] at (-1.45,1.12) {Training: compute features and generate search targets};
\node[flow] (a) at (0,0) {Packing\\Compute degree\\All-low thermal solve};
\node[flow] (b) at (3.3,0) {Degree ranking\\Heat-throughput ranking\\Choose better start};
\node[flow] (c) at (6.6,0) {Swap search\\5 runs, 3000 swaps\\Selection targets};
\node[flow] (d) at (9.9,0) {Fit GNN and MLP\\Inputs: features\\Targets: search selections};
\draw[arr](a)--(b);\draw[arr](b)--(c);\draw[arr](c)--(d);
\draw[arr,dashed] (a.south)--++(0,-.4)-|node[pos=.25,below,font=\scriptsize,fill=white] {Particle features, including degree and heat throughput} (d.south);
\node[anchor=west,tumblue,font=\small\bfseries] at (-1.45,-1.65) {Prediction on a new packing: features remain necessary};
\node[flow] (e) at (0,-2.8) {New packing\\Compute degree\\All-low thermal solve};
\node[flow] (f) at (3.3,-2.8) {Trained GNN\\Particle features\\Contact features};
\node[flow] (g) at (6.6,-2.8) {Rank GNN scores\\Select 10/30/10\\high-$k$ particles};
\node[flow] (h) at (9.9,-2.8) {Thermal verification\\Selected allocation\\Obtain $k_{\rm eff}$};
\draw[arr](e)--(f);\draw[arr](f)--(g);\draw[arr](g)--(h);
\end{tikzpicture}\par\vspace{.2cm}
\hi{Study evaluation:} compare GNN with random allocation, degree ranking, heat-throughput ranking, MLP and swap search on the same held-out packing.
