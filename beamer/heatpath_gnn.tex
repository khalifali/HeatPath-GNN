...
\small
\begin{tabular}{p{2.0cm}p{10.6cm}}\toprule Method & Selection rule within each size class\\\midrule
Random & Draw 10 / 30 / 10 particles at random. Repeat 100 times and average the resulting bed conductivities.\\[.25cm]
Degree & Count each particle's contacting neighbours. Select the 10 / 30 / 10 particles with the highest counts.\\[.25cm]
\shortstack[l]{Heat\\throughput} & First solve the all-low-conductivity bed. Select the 10 / 30 / 10 particles with the largest baseline heat-throughput scores.\\\bottomrule\end{tabular}
\vspace{.4cm}\result{Each allocation gives a list of high-conductivity particle IDs, then a thermal solve gives $k_{\rm eff}$.}
\small Existing result plots label heat-throughput ranking as ``Heat path''.
\end{frame}
\begin{frame}{Swap search starts from the better reference allocation}
\small
\begin{enumerate}
\item Evaluate the degree and heat-throughput allocations.\par Start from whichever gives the larger $k_{\rm eff}$.
...
\end{frame}
\begin{frame}{Baseline physics before material allocation}
\small
\hi{Homogeneous means identical material conductivity}
...
\end{frame}
\begin{frame}{The GNN uses geometry and the baseline thermal solution}
\small
\begin{columns}[T,onlytextwidth]\column{.48\textwidth}\hi{Eight particle features}\begin{itemize}\item Diameter, height and contact degree\item Hot-wall, cold-wall and anchored flags\item Normalized baseline temperature\item Normalized baseline heat-throughput score\end{itemize}\column{.48\textwidth}\hi{Six contact features}\begin{itemize}\item Contact radius\item Relative displacement in $x,y,z$\item Absolute vertical alignment\item Normalized absolute baseline contact heat rate\end{itemize}\end{columns}\vspace{.3cm}\small Three message-passing layers, width 64. MLP uses the same particle features. GNN additionally uses neighbour messages and contact features.
\end{frame}
\begin{frame}{Training targets describe how often search selects each particle}
\small
\begin{columns}[T,onlytextwidth]\column{.48\textwidth}\hi{A soft selection target}
\[y_i=\frac{\text{search runs selecting particle }i}{5}\]
Example: selection in four of five runs gives $y_i=0.8$.\par\vspace{.3cm}
The network predicts a score $z_i$ and $p_i=\mathrm{sigmoid}(z_i)$.\column{.48\textwidth}\hi{Two parts of the training loss}
\[L=L_{\rm ranking}+0.2L_{\rm BCE}\]
Ranking: favour particles frequently selected by search, within each size class.\par\vspace{.2cm}
Binary cross entropy (BCE): penalize disagreement between $p_i$ and $y_i$.\end{columns}\vspace{.3cm}\result{The model learns particle priorities. Exact quotas are enforced after scoring.}
\end{frame}
\begin{frame}{Scoring assigns priorities, then ranking enforces the budget}
\small
\begin{enumerate}
\item Run the trained model to obtain one numerical score per particle.\par A higher score means higher priority for high conductivity.
\item Convert scores to ranks within each size class, for each of three trained repeats.
\item Average those ranks and select the top 10 / 30 / 10 particles.
\item Assign $k_{\rm high}=10$ to those particles and solve the thermal network to verify $k_{\rm eff}$.
\end{enumerate}\vspace{.3cm}\result{Fifty selected particles, regardless of how many predicted probabilities exceed 0.5.}
\end{frame}
